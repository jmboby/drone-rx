# Replicated Product Feedback — DroneRx
...
- **`when`-gated ConfigOptions return empty string, not the declared default — diverges silently from KOTS/legacy EC.** If a config item is hidden, `ConfigOption "foo"` returns `""` rather than the `default:` you configured. In dronerx this only surfaced at the boundary with Helm's `values.schema.json` — which validates the full values document *before* any template `if` runs, so `if`-guarded consumers don't save you. An unscoped `minimum: 1` on `postgresql.instances` failed because hidden `postgres_instances` rendered as `""` → `ParseInt` → `0`. Worked around with `| default "1"` in two places in the HelmChart CR; the cleaner fix is scoping the schema with `if/then` on `postgresql.enabled`. The behavior is intentional in EC v3 (per the `config-defaults-fix.md` proposal — prevents stale persisted user values leaking through) but is undocumented and differs from KOTS/legacy EC, so vendors porting Config YAML across hit it as a silent surprise.
